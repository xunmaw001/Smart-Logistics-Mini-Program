	...
	<script>
		export default {
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('cheliang', this.cheliangId);
                    this.detail = res.data
					this.detail.cheliangContent = this.detail.cheliangContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
                                ...
                                // uni.setStorageSync('cheliangId', _this.cheliangId);
                                // _this.$utils.jump('/pages/cheliangYuyue/add-or-update');

								await _this.$api.add('cheliangYuyue', {
									yonghuId: _this.user.id,
									cheliangId: _this.detail.id,
								});
								_this.$utils.msg('操作成功');
								_this.$utils.jump('/pages/cheliangYuyue/list');
							}
						}
					});
				},
