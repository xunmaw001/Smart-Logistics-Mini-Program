	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 0 0px rgba(0,0,0,.6) inset","margin":"0","borderColor":"var(--publicMainColor)","backgroundColor":"#fff","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderRadius":"0","borderWidth":"2rpx 2rpx 2rpx 8rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.wuliuText" type="text" placeholder="物流路线"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('wuliu','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('wuliu','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
        ...
        <view style="padding: 0 20rpx;width: 100%;margin: 0 auto;;flex-direction: column;flex-wrap: wrap;flex-direction: row;margin-top: 0upx;">
            <view class="list b-b" v-for="(item, index) in list" :key="index" style="width:100%;height: auto;border: 1rps solid #999;margin: 0 auto;background-color: #FFFFFF;margin-top: 10rpx; box-shadow: 0  0 10rpx 0 #999999;border-radius: 25rpx;">
                <view
                    ...
                    <h4>{{ item.wuliuName }}</h4>
                    <view>
                        <uni-icons v-show="isAuth('wuliu','删除')" @tap="onDeleteTap(item.id)" type="closeempty"
                                   size="20"></uni-icons>
                    </view>
                </view>
                <!-- 级联表 -->
                ...
                <!-- 当前表 -->
                 <view v-show="item.wuliuCourierNumber" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">物流单号</text>
                     <text style="width: 70%">
                         <text v-if="item.wuliuCourierNumber != null">{{item.wuliuCourierNumber}}</text>
                         <text v-else></text>
                     </text>
                 </view>
                <view v-show="item.wuliuText" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">物流路线</text>
                    <view style="width: 70%" v-html="item.wuliuText"></view>
                </view>
                <view v-show="item.wuliuContent" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">物流详情</text>
                    <view style="width: 70%" v-html="item.wuliuContent"></view>
                </view>
            <view style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;font-size: 24rpx;color: #8799A3;">
                ...
                <uni-icons v-show="isAuth('wuliu','修改')" @tap="onUpdateTap(item.id)" type="compose" size="20">
                </uni-icons>
                <uni-icons v-show="isAuth('wuliu','回复')" @tap="onReply(item.id)" type="chat-filled" size="20">
                </uni-icons>
                <!--<view v-if="isAuth('wuliu','审核') && item.wuliuYesnoTypes==1" class="" style="display: flex; width: 70%;color: #FFFFFF;">
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let _this = this
                let res = await this.$api.page(`wuliu`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('wuliu', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            data.wuliuShenheTime=shijian;
                            await _this.$api.update(`wuliu`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.wuliuText) {
                    searchForm['wuliuText'] = this.searchForm.wuliuText
                }

                let res = await this.$api.page(`wuliu`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                    if(item.wuliuContent!=null){
                        item.wuliuContent = item.wuliuContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+_this.$base.url+"upload/");
