	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 0 0px rgba(0,0,0,.6) inset","margin":"0","borderColor":"var(--publicMainColor)","backgroundColor":"#fff","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderRadius":"0","borderWidth":"2rpx 2rpx 2rpx 8rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.cheliangName" type="text" placeholder="车辆名称"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('cheliang','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('cheliang','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
                ...
                <scroll-view scroll-y="true" class="tabView" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"10rpx 0 0 ","borderColor":"rgba(0,0,0,1)","borderRadius":"0","borderWidth":"0","borderStyle":"solid","height":"auto"}'>
                    <view style="display: flex;">
                        <view style="display: flex;flex-direction: column;width: 100%;">
                            <view :style='cheliangTypes===item.codeIndex?sactiveItem:sitem' class="tab"
                                  v-for="(item,index) in cheliangTypesList" :key="index" :class="cheliangTypes===item.codeIndex?'tabActive':''"
                                  @tap="cheliangTypesCategoryClick(item.codeIndex)">
                                {{item.indexName}}
                            </view>
                        </view>
                    </view>
        ...
        <view style="padding: 0 20rpx;width: 100%;margin: 0 auto;;flex-direction: column;flex-wrap: wrap;flex-direction: row;margin-top: 0upx;">
            <view class="list b-b" v-for="(item, index) in list" :key="index" style="width:100%;height: auto;border: 1rps solid #999;margin: 0 auto;background-color: #FFFFFF;margin-top: 10rpx; box-shadow: 0  0 10rpx 0 #999999;border-radius: 25rpx;">
                <view
                    ...
                    <h4>{{ item.cheliangName }}</h4>
                    <view>
                        <uni-icons v-show="isAuth('cheliang','删除')" @tap="onDeleteTap(item.id)" type="closeempty"
                                   size="20"></uni-icons>
                    </view>
                </view>
                <!-- 级联表 -->
                <!-- 当前表 -->
                 <view v-show="item.cheliangUuidNumber" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">车辆编号</text>
                     <text style="width: 70%">
                         <text v-if="item.cheliangUuidNumber != null">{{item.cheliangUuidNumber}}</text>
                         <text v-else></text>
                     </text>
                 </view>
                 <view v-show="item.cheliangValue" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">车辆类型</text>
                     <text style="width: 70%">{{item.cheliangValue}}</text>
                 </view>
                 <view v-show="item.zhuangtaiValue" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">车辆状态</text>
                     <text style="width: 70%">{{item.zhuangtaiValue}}</text>
                 </view>
                <view v-show="item.cheliangContent" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">车辆详情</text>
                    <view style="width: 70%" v-html="item.cheliangContent"></view>
                </view>
            <view style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;font-size: 24rpx;color: #8799A3;">
                ...
                <uni-icons v-show="isAuth('cheliang','修改')" @tap="onUpdateTap(item.id)" type="compose" size="20">
                </uni-icons>
                <uni-icons v-show="isAuth('cheliang','回复')" @tap="onReply(item.id)" type="chat-filled" size="20">
                </uni-icons>
                <!--<view v-if="isAuth('cheliang','审核') && item.cheliangYesnoTypes==1" class="" style="display: flex; width: 70%;color: #FFFFFF;">
...
<script>
	export default {
		...
		methods: {
            ...
            //当前表的 车辆类型字段类别搜索
			cheliangTypesCategoryClick(categoryName) {
			...
			async upCallback(mescroll) {
                ...
                if (this.cheliangTypes != 0) {
                    params.cheliangTypes = this.cheliangTypes
                }
                let _this = this
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            data.cheliangShenheTime=shijian;
                            await _this.$api.update(`cheliang`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.cheliangName) {
                    searchForm['cheliangName'] = this.searchForm.cheliangName
                }
