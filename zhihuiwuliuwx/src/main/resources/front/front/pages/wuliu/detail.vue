	...
	<script>
		export default {
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('wuliu', this.wuliuId);
                    this.detail = res.data
					this.detail.wuliuContent = this.detail.wuliuContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
					this.swiperList = this.detail.wuliuPhoto ? this.detail.wuliuPhoto.split(",") : [];
				},
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
                                ...
                                // uni.setStorageSync('wuliuId', _this.wuliuId);
                                // _this.$utils.jump('/pages/wuliuYuyue/add-or-update');

								await _this.$api.add('wuliuYuyue', {
									yonghuId: _this.user.id,
									wuliuId: _this.detail.id,
								});
								_this.$utils.msg('操作成功');
								_this.$utils.jump('/pages/wuliuYuyue/list');
							}
						}
					});
				},
