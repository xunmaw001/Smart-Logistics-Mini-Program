	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 0 0px rgba(0,0,0,.6) inset","margin":"0","borderColor":"var(--publicMainColor)","backgroundColor":"#fff","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderRadius":"0","borderWidth":"2rpx 2rpx 2rpx 8rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.sijiName" type="text" placeholder="司机名称"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('siji','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('siji','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
                ...
                <scroll-view scroll-y="true" class="tabView" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"10rpx 0 0 ","borderColor":"rgba(0,0,0,1)","borderRadius":"0","borderWidth":"0","borderStyle":"solid","height":"auto"}'>
                    <view style="display: flex;">
                        <view style="display: flex;flex-direction: column;width: 100%;">
                            <view :style='sijiTypes===item.codeIndex?sactiveItem:sitem' class="tab"
                                  v-for="(item,index) in sijiTypesList" :key="index" :class="sijiTypes===item.codeIndex?'tabActive':''"
                                  @tap="sijiTypesCategoryClick(item.codeIndex)">
                                {{item.indexName}}
                            </view>
                        </view>
                    </view>
        ...
        <view style="padding: 0 20rpx;width: 100%;margin: 0 auto;;flex-direction: column;flex-wrap: wrap;flex-direction: row;margin-top: 0upx;">
            <view class="list b-b" v-for="(item, index) in list" :key="index" style="width:100%;height: auto;border: 1rps solid #999;margin: 0 auto;background-color: #FFFFFF;margin-top: 10rpx; box-shadow: 0  0 10rpx 0 #999999;border-radius: 25rpx;">
                <view
                    ...
                    <h4>{{ item.sijiName }}</h4>
                    <view>
                        <uni-icons v-show="isAuth('siji','删除')" @tap="onDeleteTap(item.id)" type="closeempty"
                                   size="20"></uni-icons>
                    </view>
                </view>
                <!-- 级联表 -->
                <!-- 当前表 -->
                 <view v-show="item.sijiPhone" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">联系方式</text>
                     <text style="width: 70%">
                         <text v-if="item.sijiPhone != null">{{item.sijiPhone}}</text>
                         <text v-else></text>
                     </text>
                 </view>
                 <view v-show="item.sijiEmail" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">邮箱</text>
                     <text style="width: 70%">
                         <text v-if="item.sijiEmail != null">{{item.sijiEmail}}</text>
                         <text v-else></text>
                     </text>
                 </view>
                 <view v-show="item.sijiValue" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">司机类型</text>
                     <text style="width: 70%">{{item.sijiValue}}</text>
                 </view>
                <view v-show="item.sijiContent" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">司机介绍</text>
                    <view style="width: 70%" v-html="item.sijiContent"></view>
                </view>
            <view style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;font-size: 24rpx;color: #8799A3;">
                ...
                <uni-icons v-show="isAuth('siji','修改')" @tap="onUpdateTap(item.id)" type="compose" size="20">
                </uni-icons>
                <uni-icons v-show="isAuth('siji','回复')" @tap="onReply(item.id)" type="chat-filled" size="20">
                </uni-icons>
                <!--<view v-if="isAuth('siji','审核') && item.sijiYesnoTypes==1" class="" style="display: flex; width: 70%;color: #FFFFFF;">
...
<script>
	export default {
		...
		methods: {
            ...
            //当前表的 司机类型 字段 字典表查询方法
            async sijiTypesSelect(){
                let sijiTypes = await this.$api.page('dictionary', {
                    page: 1,
                    limit: 100,
                    dicCode:'siji_types',
                });
                sijiTypes.data.list.splice(0, 0, {codeIndex: 0, indexName: '全部'})
                this.sijiTypesList = sijiTypes.data.list
            },
            //当前表的 司机类型字段类别搜索
			sijiTypesCategoryClick(categoryName) {
			...
			async upCallback(mescroll) {
                ...
                if (this.sijiTypes != 0) {
                    params.sijiTypes = this.sijiTypes
                }
                let _this = this
                let res = await this.$api.list(`siji`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('siji', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            data.sijiShenheTime=shijian;
                            await _this.$api.update(`siji`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.sijiName) {
                    searchForm['sijiName'] = this.searchForm.sijiName
                }

                let res = await this.$api.list(`siji`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                    if(item.sijiContent!=null){
                        item.sijiContent = item.sijiContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+_this.$base.url+"upload/");
