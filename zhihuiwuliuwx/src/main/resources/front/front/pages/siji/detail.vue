	...
	<script>
		export default {
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('siji', this.sijiId);
                    this.detail = res.data
					this.detail.sijiContent = this.detail.sijiContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
					this.swiperList = this.detail.sijiPhoto ? this.detail.sijiPhoto.split(",") : [];
				},
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
                                ...
                                // uni.setStorageSync('sijiId', _this.sijiId);
                                // _this.$utils.jump('/pages/sijiYuyue/add-or-update');

								await _this.$api.add('sijiYuyue', {
									yonghuId: _this.user.id,
									sijiId: _this.detail.id,
								});
								_this.$utils.msg('操作成功');
								_this.$utils.jump('/pages/sijiYuyue/list');
							}
						}
					});
				},
